...
from devsearch.settings import N, TBA, EMAIL_HOST_USER, ALLOWED_HOSTS
...
def get_pwd_imgs():
    # select randomly N number of images
    #N = 5
    images = random.sample(range(1, 41), N * N)
    print(images)
    p_images = []
...
def sendLoginLinkMailToUser(username):
    ...
    if user.logininfo.login_link is None:
        ...
        email = EmailMessage(
            ...
            body='''
            An attempted brute force attack has been detected on your account !. 
            Please use the provided link to access your account securely. Remember, 
            this link can only be used once.
            link: http://{}:8000/login/{}
            '''.format('localhost',link), 
...
def sendPasswordResetLinkToUser(username):
    ...
    email = EmailMessage(
        ...
        body='''
        You've made a request to reset your password. 
        Please use the link provided to reset your password directly.
        Keep in mind that this link can only be used once.
...
def home_page(request):
    return render(request,'login_gp.html')
...
def reg_gp(request):
    if request.method == 'POST':
        ...
        return redirect('login_gp')
    else:
        data = {
            'p_images': get_pwd_imgs(),
        }
        return render(request, 'reg_gp.html', context=data)


def login_page_gp(request):
    if request.method == 'POST':
        username = request.POST['username']
        password = request.POST['password']
        print(username, password)
        ...
        return render(request, 'login_gp.html', context=data)


def login_from_uid_gp(request, uid):
    try:
...
def reset_view_gp(request):
    if request.method == 'POST':
        ...
        return render(request, 'reset_request_gp.html')


def reset_from_uid_gp(request, uid):
    print('hello')
    if request.method == 'POST':
        print('hi-post')
        password = request.POST['password']
        ...
        try:
            ...
            return render(request, 'reset_gp.html', context=data)
        except Exception:
            messages.warning(request, 'Invalid Link. Please check again!')
            return redirect('home')


def logout_page_gp(request):
    logout(request)
